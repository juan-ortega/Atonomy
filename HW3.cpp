...
int main()
{
	...
	for (int oh = 0; oh < numofruns; oh++) {
		...
		if (yprize != 0) {
			...
			grid.at(wall_1).position = 2;
			prizes.at(wall_1).value = -1000;
		}
		if (yprize != y - 1) {
			int wall_2 = coord(xprize, yprize + 1);
			grid.at(wall_2).position = 2;
			prizes.at(wall_2).value = -1000;
		}
		if (xprize != 0) {
			if (xprize != x - 1) {
				int wall_3;
				...
				grid.at(wall_3).position = 2;
				prizes.at(wall_3).value = -1000;
			}
		}
		...
		for (int m = 0; m < cycles; m++) {
			...
			while (agentcoord != prizecoord) {
				...
				if (grid.at(agentcoord).position == 2) {///////////////////
					agentcoord = agentcoordsaver;
				}
